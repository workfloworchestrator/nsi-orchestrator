...
def terminate_initial_input_form_generator(subscription_id: UUIDstr, customer_id: UUIDstr) -> InputForm:
    ...
    return TerminateServiceDemarcationPointForm


@step("Terminate service demarcation point subscription")
def terminate_sdp_subscription(subscription: ServiceDemarcationPoint) -> State:
    # DDS is read-only, so there is nothing to deprovision; just drop the subscription.
    logger.info("Terminating service demarcation point subscription", sdp_name=subscription.sdp.sdp_name)

    return {}


@terminate_workflow(initial_input_form=terminate_initial_input_form_generator)
def terminate_sdp() -> StepList:
    return begin >> terminate_sdp_subscription
